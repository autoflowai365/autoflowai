    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#050B18]/95 backdrop-blur-xl shadow-2xl border-b border-white/10" : "bg-slate-950/80 backdrop-blur-md border-b border-slate-800"}`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 md:px-6 py-3 md:py-4">
        ...
        <nav className="hidden md:flex gap-8 text-slate-200">
          ...
          <Link href="/pricing" className="text-gray-300 transition-all duration-300 hover:text-blue-400 text-sm">Pricing</Link><Link href="/demo" className="text-blue-400 transition-all duration-300 hover:text-blue-300 text-sm font-semibold border border-blue-500/30 px-3 py-1 rounded-full">Live Demo</Link>
      ...
      {open && (
        <div className="md:hidden bg-slate-900 border-t border-slate-800">
          ...
          <button onClick={() => handleNav("/pricing")} className="block w-full text-left px-4 py-3 text-white hover:bg-slate-800 transition text-sm">Pricing</button>
