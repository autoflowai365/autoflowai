      ...
      <div className="max-w-7xl mx-auto px-6 relative">
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {steps.map((step, i) => (
              <div key={i} className={`group relative rounded-3xl border p-10 hover:-translate-y-2 hover:shadow-[0_0_40px_rgba(37,99,235,0.25)] transition-all duration-300 ${step.highlight ? "border-blue-500 bg-blue-600/10 shadow-[0_0_30px_rgba(37,99,235,0.2)]" : "border-slate-700 bg-slate-800/50 hover:border-blue-500"}`}>
                ...
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 group-hover:rotate-6 group-hover:scale-110 transition-transform duration-300 ${step.highlight ? "bg-blue-600 text-white" : "bg-blue-600/20 text-blue-400"}`}>
                  {step.icon}
                </div>
                <h3 className="text-2xl font-bold text-white mb-2">{step.title}</h3>
                <span className="inline-block text-blue-400 text-sm font-semibold mb-4 bg-blue-600/10 px-3 py-1 rounded-full">⏱ {step.time}</span>
